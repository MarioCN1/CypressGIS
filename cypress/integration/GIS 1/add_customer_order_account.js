...
import { random_code } from "../../support/code_generator";

it('GIS Login, Add Customer and Create Order', ()=>{
   cy.Login_GIS();  
   //Add Customer
   cy.log('Add Customer'); 
   cy.get('.nav-left > :nth-child(1) > .btn-dropdown-toggle').click();
...
//Create account
  ...
  cy.get('#email').type(`${Date.now()}@newtest.com`);
  cy.get('.btn-main').click();
  cy.Logout_GIS()
})